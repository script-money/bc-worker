...
def get_hodlers_for_public_key(username: str, number_to_fetch: int = 100):
    ...
    try:
        ...
        res_json = rq.post(
            URL + "get-hodlers-for-public-key",
            json=data_dict,
            headers=HEADERS,
            proxies=proxies,
...
def find_investors(res_json: dict) -> dict[str, float]:
    ...
    return {
        holder["ProfileEntryResponse"]["Username"]: round(
            holder["BalanceNanos"] / 1e9, 4
        )
        for holder in holders
        if holder["ProfileEntryResponse"] is not None
    }
...
def get_notification(
    public_key: str, number_to_fetch: int = 50, fetch_start_index: int = -1
):
    data_dict = {
        "PublicKeyBase58Check": public_key,
