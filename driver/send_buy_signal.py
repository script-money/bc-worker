...
def main():
    ...
    while True:
        try:
            ...
            twitter_check_list = filter_newest_tweets(
                last_response, new_response)
            last_response = new_response
            if len(twitter_check_list) != 0:
                usernames = get_bitclout_username(
                    filter(filter_tweets, twitter_check_list))  # filter
                ...
                if len(usernames) != 0:
                    for username in usernames:
                        ...
                        if interval > cooldown:
                            ...
                            logger.info(
                                f"newest request timestamp is {last_timestamp}")
                        else:
                            wait_seconds = cooldown-interval
                            logger.info(
                                f"sleep {wait_seconds} seconds...")
