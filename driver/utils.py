...
def filter_newest_tweets(old: TwitterResponse, new: TwitterResponse) -> list[TwitterResponse]:
    """
    两次请求获取新的tweets
...
def filter_tweets(i: TwitterResponse) -> bool:
    ...
    Returns:
        bool: 是否满足条件
    """
    created_at = convert_timestr_to_timestruct(
        i['user']['created_at']).tm_year < 2021  # 2021年前创建
    # 粉丝数是关注数2倍以上
...
def get_bitclout_username(r: TwitterResponse) -> set[str]:
    ...
    for index, tweet in enumerate(r):
        ...
        regex_url = re.compile(
            r"'expanded_url': 'https:\/\/bitclout.com\/u\/(\w+)")
